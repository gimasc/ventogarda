#!/usr/bin/env python3
"""
VERSIONE DI PROVA di update_meteo_cache.py — NON tocca i file usati dal sito.
Novita' rispetto alla produzione:
  - vento medio (wind_speed_10m) per ogni spot, stessa chiamata delle raffiche
  - blocco icon2i (vento ICON-2I / modello Meteotrentino) sul centro localita'
Output SEPARATO:
  data/test/meteo_torbole.json
  data/test/meteo_malcesine.json
Quando la prova e' conclusa, queste modifiche vanno riportate nello script
di produzione e questo file puo' essere eliminato.
...
def genera_localita(loc_id: str, cfg: dict) -> dict:
    ...
    #    (wind_speed_10m aggiunto per il futuro grafico del vento medio:
    #    stessa chiamata di prima, solo un parametro in piu' — zero chiamate extra)
    spots_data = []
    ...
    # 3. Vento ICON-2I (modello di Meteotrentino) sul centro della localita'.
    #    Serve per l'Indice Vento (interpolazione). Opzionale: se fallisce,
    #    il resto della cache viene comunque salvato (icon2i = None).
    icon2i_data = None
    try:
        d2i = fetch({
            "latitude": cfg["lat_meteo"], "longitude": cfg["lon_meteo"],
